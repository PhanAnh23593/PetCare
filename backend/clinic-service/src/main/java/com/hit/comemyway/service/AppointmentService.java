...
public class AppointmentService {
  ...
  public AppointmentResponse createAppointment(AppointmentRequest request) {
    ...
    return response(savedAppointment);
  }
  ...
  private void requireOwner(Appointment appointment) {
    String username = SecurityContextHolder.getContext().getAuthentication().getName();
    UserRef user = userRepository.findByUsername(username).orElseThrow();
    boolean owns = user.getRole() == com.hit.comemyway.entity.Role.CLINIC
        ? appointment.getClinic().getUser().getId().equals(user.getId())
  ...
  public List<AppointmentResponse> getPendingStatusAppointment() {
    ...
    Clinic clinic = clinicRepository
        .findByUserId(userRepository.findByUsername(username)
            .orElseThrow(() -> new AppException(404, ErrorMessage.User.USER_NOT_EXISTED)).getId())
        .orElseThrow(() -> new AppException(404, ErrorMessage.Clinic.CLINIC_NOT_EXISTED));

    List<Appointment> appointments = appointmentRepository
        .findPendingAppointmentsByClinicId(clinic.getId(), BookingStatus.PENDING);

    return appointments.stream().map(this::response).toList();
  }
