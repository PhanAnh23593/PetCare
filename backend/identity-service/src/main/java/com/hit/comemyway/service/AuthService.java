...
public class AuthService {
  ...
  public LoginResponse login(LoginRequest request) {
    ...
    return new LoginResponse(accessToken, refreshToken, user.getId(), user.getRole(),
        user.getStatus());
  }
  ...
  public RegisterResponse verifyRegister(VerifyOtpRequest request) {
    ...
    try {
      ...
      User user = User.builder().username(registerRequest.username()).password(password)
          .email(registerRequest.email()).role(Role.USER).locketCode(locketCode).homeAddress("")
          .status(com.hit.comemyway.entity.AccountStatus.ACTIVE).build();
  ...
  public LoginResponse refreshToken(RefreshTokenRequest request) {
    ...
    return new LoginResponse(newAccessToken, newRefreshToken, user.getId(), user.getRole(),
        user.getStatus());
  }
}
