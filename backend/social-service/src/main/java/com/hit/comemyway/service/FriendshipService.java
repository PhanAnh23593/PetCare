...
public class FriendshipService {
  ...
  private final IdentityClient userRepository;
  private final SecurityUtils securityUtils;

  private FriendResponse response(Friendship friendship, Long currentUserId) {
    Long friendId =
        friendship.getUser().getId().equals(currentUserId) ? friendship.getFriend().getId()
            : friendship.getUser().getId();
    UserRef friend = userRepository.findById(friendId)
        .orElseThrow(() -> new AppException(404, ErrorMessage.User.USER_NOT_EXISTED));
    return new FriendResponse(friendship.getId(), friend.getId(), friend.getUsername(),
        friend.getAvatar());
  }

  @Transactional
  public FriendResponse addFriendRequest(FriendRequest request) {
    UserRef user = securityUtils.getCurrentUser();

    if (user.getId().equals(request.friendId())) {
      throw new AppException(400, ErrorMessage.Locket.CANNOT_ADD_FRIEND_TO_YOURSELF);
    }

    UserRef friend = userRepository.findById(request.friendId())
        .orElseThrow(() -> new AppException(404, ErrorMessage.User.USER_NOT_EXISTED));
    ...
    return response(savedFriendship, user.getId());
  }
  ...
  public List<FriendResponse> findPendingFriendship() {
    ...
    return pendingRequests.stream().map(f -> response(f, currentUser.getId())).toList();
  }
  ...
  public List<FriendResponse> getAcceptedFriends() {
    ...
    return friendships.stream().map(f -> response(f, currentUser.getId())).toList();
  }
  ...
  public FindFriendResponse findFriend(FindFriendRequest request) {
    ...
    UserRef user = userRepository.findBylocketCode(locketCode)
        .orElseThrow(() -> new AppException(404, ErrorMessage.User.USER_NOT_EXISTED));
